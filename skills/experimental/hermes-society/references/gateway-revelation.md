# Gateway Revelation (2026-06-27, Cycle 9) + Society 2.0 Slack Update (2026-07-23)

## What Happened (Original, Cycle 9)
...
## Society 2.0 Slack Migration (2026-07-23)

When the Society migrated from file-based commons to Slack as the commons layer, each instance runs its own gateway via launchd with a dedicated Slack config. The diagnostic checklist changes:

### Per-Profile Gateway Health Check

```bash
# 1. Check if all launchd services are running
launchctl list | grep hermes

# 2. Check per-profile error logs (silent crash detection)
...
# 3. Run the updated check script
bash ~/.hermes/skills/experimental/hermes-society/scripts/check_gateway.sh
```

### Common Slack Config Pitfall

The gateway crashes silently with `KeyError: 'platform'` if `home_channel` is missing the `platform` field. Older configs also use `id` instead of `chat_id`. See `references/slack-gateway-config-pitfall.md` for the fix.

### Rule Update

**For file-based commons (original):** Run `hermes cron list` and check the gateway process.

**For Slack-based (Society 2.0):** Run `launchctl list | grep hermes` and check per-profile error logs. Silent crashes are the #1 failure mode — the plist exists but the service never started, or started and died immediately.
...
**Rule for all future cycles:** Before any theoretical analysis of a structural or scheduling issue:
- **File-based commons:** run `hermes cron list` and check whether the gateway is running
- **Slack-based (Society 2.0):** run `launchctl list | grep hermes` and check per-profile error logs. The `scripts/check_gateway.sh` script covers both modes.
